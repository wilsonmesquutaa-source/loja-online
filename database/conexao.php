...
$porta = (string) ($_ENV['DB_PORT'] ?? '3306');
$banco = (string) ($_ENV['DB_DATABASE'] ?? '');
$usuario = (string) ($_ENV['DB_USERNAME'] ?? 'root');
$senha = (string) ($_ENV['DB_PASSWORD'] ?? '');
...
try {
    ...
    error_log('[CONEXÃO COM O BANCO] ' . $erro->getMessage());
